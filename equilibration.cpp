...
int main(int argc , char **argv){
	...
	file.open("initial_parameters.txt", ios::in);
    ...
    results.open("sim_results.txt", ios::out);
    	if(results.fail()){
		cout<<"$$ | RESULTS IS BROKEN | $$"<<endl;
		exit(1);
	}
...
void verlet(box* BOX , double dt){
    ...
    compute_acc(& BOX); //DE AQUI SALE U
    for (int i = 0; i < N;i++){
        for (int k = 0; k<dim;k++){
            BOX->world[i].p[k] += 0.5*BOX->world[i].a[k]*dt;
        }
    ...
    for (int i = 0 ; i < N ;i++){
        for (int k = 0; k < dim ;k++){
            BOX->K += 0.5*(BOX->world[i].p[k])*(BOX->world[i].p[k]); //K
            // AQUÍ ES DONDE PODRIA METER LOS VALORES QUE FALTAN
        }
    }
}
...
void compute_acc(box** BOX){
   ...
   ///////////////////// **************** Aquí podría meter los potenciales *************** ////////////////////////         
            for (int k = 0; k < dim; k++) {
...
/*///////////////////////////////// PREGUNTA: DONDE OSTIAS METO <Ec*Phi_{V}> 
		Sol: Añadir en la estructura un dato Ec, phi_V, phi_VV para cada partícula  /////////////////////
